...
export const EmergencyContact = () => {
  const user: User | null = JSON.parse(
    localStorage.getItem("mama_user") || "null"
  );
  ...
  const sendEmergencyMessage = async () => {
    ...
    try {
      const dueDate =
        user?.pregnancyData?.dueDate || user?.dueDate
          ? new Date(user.pregnancyData?.dueDate || user.dueDate!)
      ...
      const response = await fetch(
        "https://l2nemupx35qeg4ke2yzsbjoody0hkxbj.lambda-url.eu-west-1.on.aws/",
        {
          method: "POST",
          headers: {
      ...
      if (responseBody.audio_base64) {
        const audio = new Audio(
          `data:audio/mp3;base64,${responseBody.audio_base64}`
        );
        audio.play();
      }
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Send an urgent message to emergency services
          </p>
          {user && (
        ...
        <div className="text-center mt-8">
          ...
          <p className="text-xs text-gray-500">
            Your safety is our priority. Emergency services will be contacted
            immediately.
          </p>
        </div>
      </div>
