...
class CronService {
  ...
  async sendWeeklyInsights() {
    try {
      ...
      for (const user of users) {
        try {
          const insights = await this.getInsightsForUser(user.pregnancyData.dueDate);
          const message = `MAMA Weekly Update: ${insights.baby_this_week.substring(0, 140)}...`;
  ...
  async getInsightsForUser(dueDate) {
    ...
    const response = await axios.post('https://gh47sa3nnjjkhmexbhciqgks4a0xfunk.lambda-url.us-west-2.on.aws/', {
      due_month: dueMonth,
      due_year: dueYear
    });
